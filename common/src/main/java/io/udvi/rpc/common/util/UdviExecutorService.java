package io.udvi.rpc.common.util;
 ...
 */
public class UdviExecutorService extends AbstractExecutorService {
  ...
  private final Logger logger = LoggerFactory.getLogger(UdviExecutorService.class);
   ...
   */
  public UdviExecutorService(BlockingQueue<Runnable> workQueue, ExecutorService executor, String poolName, int maxDrainers, long maxTimeSlice, TimeUnit maxTimeSliceUnit) {
  ...
  public UdviExecutorService(BlockingQueue<Runnable> workQueue, ExecutorService executor, int maxDrainers, long maxTimeSlice, TimeUnit maxTimeSliceUnit) {
    this(workQueue, executor, "Drainer", maxDrainers, maxTimeSlice, maxTimeSliceUnit);
  }

  public UdviExecutorService(BlockingQueue<Runnable> workQueue, ExecutorService executor, String poolName, int maxDrainers) {
    this(workQueue, executor, poolName, maxDrainers, Long.MAX_VALUE, TimeUnit.MILLISECONDS);
  }

  public UdviExecutorService(BlockingQueue<Runnable> workQueue, ExecutorService executor, int maxDrainers) {
    this(workQueue, executor, "Drainer", maxDrainers, Long.MAX_VALUE, TimeUnit.MILLISECONDS);
  }

  public UdviExecutorService(ExecutorService executor, long maxTimeSlice, TimeUnit maxTimeSliceUnit) {
    this(new LinkedBlockingQueue<Runnable>(), executor, "Drainer", 1, maxTimeSlice, maxTimeSliceUnit);
  }
  
  public UdviExecutorService(ExecutorService executor) {
